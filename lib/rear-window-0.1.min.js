...
var siblings=parent.getElementsByTagName("li");for(var i=0;i<siblings.length;i++)removeClass(siblings[i],"selected");addClass(this,"selected");var new_column;if(type=="category"){var obj=eval(dotPath.join('.'));var mock_obj={};for(i in obj){try{var val=obj[i];if(realTypeOf(val)==category){mock_obj[i]=val;}}catch(e){}}
new_column=addColumn(mock_obj);}else{dotPath.push(this.innerHTML);new_column=addColumn(dotPath.join('.'));}
rearwindow.appendChild(new_column);resize();}
var resize=function(){var width=0;var columns=rearwindow.getElementsByTagName('div');for(var i=0;i<columns.length;i++){var column=columns[i];if(!hasClass(column,'column'))continue;var styles=column.currentStyle||getComputedStyle(column,null);width+=(parseInt(styles.width)+window.scrollBarWidth);console.log(parseInt(styles.width));}
...
window.scrollBarWidth=function(){document.body.style.overflow='hidden';var width=document.body.clientWidth;document.body.style.overflow='scroll';width-=document.body.clientWidth;if(!width)width=document.body.offsetWidth-document.body.clientWidth;document.body.style.overflow='';return width;}();var init=function(obj){var e=document.createElement('div');e.id='rear-window';e.style.top=[document.body.scrollTop+20,'px'].join('');e.innerHTML=['<div class="column focus"><ul><li class="selected">',obj,'</li></ul></div>'].join('');e.appendChild(addColumn(obj));document.body.appendChild(e);window.rearwindow=e;}
if(typeof(window.rearwindow)!='undefined'){document.body.removeChild(window.rearwindow);var old_css;if(old_css=document.getElementById('rear-window-css')){document.body.removeChild(old_css);}}
...
var css=document.createElement('link');css.rel='stylesheet';css.href=css_href;css.type='text/css';document.body.appendChild(css);var cssloader=document.createElement('div');cssloader.id='rear-window-cssloading';document.body.appendChild(cssloader);if(typeof(initial_object)=='undefined'||typeof(initial_object)=='object'){initial_object='window';}
cssOnload('rear-window-cssloading',function(){init(initial_object);});}